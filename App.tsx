...
const App: React.FC = () => {
  ...
  const DONATION_URL = 'https://pay.cloudtips.ru/p/b93104a4';
  const WEBSITE_URL = 'https://vladimirkisarov.ru/';
  ...
  const handleCreateScene = async () => {
    ...
    try {
      ...
      setInputText(''); // Стираем только при успехе
      setView('EDIT_ROLES');
    } catch (err) {
      alert('Не удалось распознать текст. Попробуйте изменить формат (например, ИМЯ: Текст) или проверьте интернет-соединение.');
    } finally {
      setIsParsing(false);
    }
  };
  ...
  const renderNewScene = () => (
    <div className="max-w-2xl mx-auto p-4 animate-in slide-in-from-bottom duration-300">
      ...
      <p className="text-slate-400 mb-8 text-lg font-medium opacity-80 leading-relaxed">
        Вставьте текст из вашего файла. Имена персонажей лучше выделять двоеточием или новой строкой.
      </p>
      ...
      <div className="space-y-10 text-slate-300">
        ...
        <div className="p-16 bg-gradient-to-br from-blue-900/40 via-slate-800/50 to-slate-900/80 rounded-[4.5rem] border-2 border-blue-500/20 text-center shadow-[0_50px_100px_-20px_rgba(0,0,0,0.5)]">
          ...
          <div className="flex flex-col sm:flex-row gap-6 justify-center">
            <Button variant="primary" size="md" onClick={() => window.open(WEBSITE_URL, '_blank')} className="px-14 h-20 rounded-3xl font-black text-xl shadow-2xl shadow-blue-600/30">
              Сайт автора
            </Button>
